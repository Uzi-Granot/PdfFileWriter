...
public class PdfContents : PdfObject
	{
	internal bool PageContents;	// true for page contents, false for X objects and pattern
	...
	public void TranslateScaleRotate
		...
		ObjectValueFormat("{2} {3} {4} {2} {0} {1} cm\n",
			ToPt(OriginX), ToPt(OriginY), Round(Scale * Math.Cos(Rotate)), Round(Scale * Math.Sin(Rotate)), Round(Scale * Math.Sin(-Rotate)));
		return;
		}
	...
	public void TranslateScaleRotate
		...
		ObjectValueFormat("{2} {3} {4} {5} {0} {1} cm\n",
			ToPt(OriginX), ToPt(OriginY), Round(ScaleX * Math.Cos(Rotate)), Round(ScaleY * Math.Sin(Rotate)), Round(ScaleX * Math.Sin(-Rotate)), Round(ScaleY * Math.Cos(Rotate)));
		return;
		}
	...
	internal double DrawTextInternal
		...
		for(int Ptr = 0; Ptr < Text.Length; Ptr++)
			...
			// output glyph index
				...
				{
				if(CharInfo.NewGlyphIndex < 0) CharInfo.NewGlyphIndex = Font.EmbeddedFont ? Font.NewGlyphIndex++ : CharInfo.GlyphIndex;
	...
	internal void OutputOneByte
		...
		switch(CharCode)
			{
			case '\r': ObjectValueList.Add((byte) '\\'); ObjectValueList.Add((byte) 'r'); return;
			case '\n': ObjectValueList.Add((byte) '\\'); ObjectValueList.Add((byte) 'n'); return;
			case '(': ObjectValueList.Add((byte) '\\'); ObjectValueList.Add((byte) '('); return;
			case ')': ObjectValueList.Add((byte) '\\'); ObjectValueList.Add((byte) ')'); return;
			case '\\': ObjectValueList.Add((byte) '\\'); ObjectValueList.Add((byte) '\\'); return;
			default: ObjectValueList.Add((byte) CharCode); return;
			}
		}
