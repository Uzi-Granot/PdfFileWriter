...
namespace PdfFileWriter
	...
	public class PdfTable
		...
		public void DrawRow
			...
			// test for new page
			if(NewPage || _RowTopPosition - (TextBoxRowHeight + (HeaderHeight != 0.0 ? (HeaderHeight + 2.0 * Borders.HeaderHorBorder.HalfWidth) : 0.0)) <
				TableBottomLimit && _RowTopPosition != TableTopLimit)
				{
				// create a new page
				CreateNewPage();
				}
		...
		private void DrawOneRow()
			...
			foreach(PdfTableCell Cell in _Cell)
				...
				if(CustomDrawCellEvent == null || !Cell.Style.RaiseCustomDrawCellEvent || !CustomDrawCellEvent(this, Cell))
					Cell.DrawCell();
				}
