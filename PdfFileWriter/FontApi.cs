...
public class FontApi : IDisposable
	...
	public CharInfo[] GetGlyphMetricsApi
		...
		// use glyph index
		bool UseGlyphIndex = FirstChar != 0;
	...
	public WinKerningPair[] GetKerningPairsApi
		...
		if(GetKerningPairs(GDIHandle, (uint) Pairs, Buffer) == 0) ThrowSystemErrorException("Calling GetKerningPairs failed");
    ...
    public int[] GetGlyphIndicesApi
		...
		for(int CharPtr = FirstChar; CharPtr <= LastChar; CharPtr++) Marshal.WriteInt16(CharBuffer, 2 * (CharPtr - FirstChar), (short) (CharPtr));

		// allocate memory for result
		AllocateBuffer(2 * CharCount);

		// get glyph numbers for all characters including non existing glyphs
		if(GetGlyphIndices(GDIHandle, CharBuffer, CharCount, Buffer, (uint) 0) != CharCount) ThrowSystemErrorException("Calling GetGlypeIndices failed");
