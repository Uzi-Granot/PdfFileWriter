...
public class PdfTableBorder
	...
	public void ClearAllBorders()
		{
		// set is not allowed
		if(Parent.Active || TopBorder == null) throw new ApplicationException("Set borders after SetColumnWidth and before table is active.");
	...
	internal double HorizontalBordersTotalWidth()
		...
		for(int Index = 0; Index <= Columns; Index++) if(HeaderVertBorder[Index].Display)
			{
		...
		for(int Index = 0; Index <= Columns; Index++) if(CellVertBorder[Index].Display)
			{
		...
		if(HeaderVertBorderActive || CellVertBorderActive)
			{
			for(int Index = 0; Index <= Columns; Index++) VertBorderHalfWidth[Index] = Math.Max(HeaderVertBorder[Index].HalfWidth, CellVertBorder[Index].HalfWidth);
