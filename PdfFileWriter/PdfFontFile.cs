...
internal class PdfFontFile : PdfObject
	...
	private void CalculateGlyphChksum()
		...
		foreach(CharInfo CharInfo in GlyphArray)
			{
			if(CharInfo.GlyphData != null) foreach(byte B in CharInfo.GlyphData) Checksum += (uint) B << (24 - 8 * (Ptr++ & 3));
			}
	...
	private void BuildLocaTable()
		...
		if(TableRecordArray[(int) Tag.glyf].Length != GlyphTableLength) throw new ApplicationException("Glyph table length does not match header");
